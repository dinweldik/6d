...
import type { ProviderRuntimeEvent, ProviderSession } from "@fatma/contracts";
import {
  CommandId,
...
function createGitRepository() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "fatma-checkpoint-handler-"));
...
describe("CheckpointReactor", () => {
  ...
  it("continues processing runtime events after a single checkpoint runtime failure", async () => {
    const nonRepositorySessionCwd = fs.mkdtempSync(
      path.join(os.tmpdir(), "fatma-checkpoint-runtime-non-repo-"),
    );
