  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* createGitBranch({ cwd: tmp, branch: "fatma/feat/session" });
        yield* createGitBranch({ cwd: tmp, branch: "fatma/tmp-working" });
        yield* checkoutGitBranch({ cwd: tmp, branch: "fatma/tmp-working" });

        const renamed = yield* renameGitBranch({
          cwd: tmp,
          oldBranch: "fatma/tmp-working",
          newBranch: "fatma/feat/session",
        });

        expect(renamed.branch).toBe("fatma/feat/session-1");
        const branches = yield* listGitBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "fatma/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "fatma/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("fatma/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* createGitBranch({ cwd: tmp, branch: "fatma/feat/session" });
        yield* createGitBranch({ cwd: tmp, branch: "fatma/feat/session-1" });
        yield* createGitBranch({ cwd: tmp, branch: "fatma/tmp-working" });
        yield* checkoutGitBranch({ cwd: tmp, branch: "fatma/tmp-working" });
        ...
        expect(renamed.branch).toBe("fatma/feat/session-2");
      }),
    );
