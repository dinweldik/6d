...
import { resolveAutoFeatureBranchName, sanitizeFeatureBranchName } from "@fatma/shared/git";
...
export const makeGitManager = Effect.gen(function* () {
  ...
  const runPrStep = (cwd: string, fallbackBranch: string | null) =>
    Effect.gen(function* () {
      ...
      const bodyFile = path.join(tempDir, `fatma-pr-body-${process.pid}-${randomUUID()}.md`);
      yield* fileSystem
