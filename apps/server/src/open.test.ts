...
describe("launchDetached", () => {
  ...
  it.effect("rejects when command does not exist", () =>
    Effect.gen(function* () {
      const result = yield* launchDetached({
        command: `fatma-no-such-command-${Date.now()}`,
        args: [],
      }).pipe(Effect.result);
...
describe("isCommandAvailable", () => {
  function withTempDir(run: (dir: string) => void): void {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "fatma-open-"));
    try {
      run(dir);
    } finally {
...
describe("resolveAvailableEditors", () => {
  it("returns only editors whose launch commands are available", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "fatma-editors-"));
    try {
