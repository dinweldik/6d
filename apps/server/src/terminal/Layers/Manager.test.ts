...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "fatma-terminal-"));
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.FATMA_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          FATMA_PROJECT_ROOT: "/repo",
          FATMA_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.FATMA_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.FATMA_WORKTREE_PATH).toBe("/repo/worktree-a");
