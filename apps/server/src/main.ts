...
const CliEnvConfig = Config.all({
  mode: Config.string("FATMA_MODE").pipe(
    Config.option,
    Config.map(
  ...
  authToken: Config.string("FATMA_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("FATMA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("FATMA_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("fatma running", {
      ...safeConfig,
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to FATMA_LOG_WS_EVENTS).",
  ),
  Flag.withAlias("log-ws-events"),
  Flag.optional,
);

export const fatmaCli = Command.make("fatma", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  ...
  Command.withDescription("Run the fatma server."),
