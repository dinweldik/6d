...
describe("wsNativeApi", () => {
  it("delivers and caches valid server.welcome payloads", async () => {
    ...
    const payload = { cwd: "/tmp/workspace", projectName: "fatma" };
  ...
  it("ignores invalid server.welcome payloads and keeps subscription active", async () => {
    ...
    emitPush(WS_CHANNELS.serverWelcome, { cwd: 42, projectName: "fatma" });
    emitPush(WS_CHANNELS.serverWelcome, { cwd: "/tmp/workspace", projectName: "fatma" });

    expect(listener).toHaveBeenCalledTimes(1);
    expect(listener).toHaveBeenCalledWith(
      expect.objectContaining({ cwd: "/tmp/workspace", projectName: "fatma" }),
    );
    expect(warnSpy).toHaveBeenCalledTimes(1);
    expect(warnSpy).toHaveBeenCalledWith("Dropped inbound WebSocket push payload", {
      reason: "decode-failed",
      raw: { cwd: 42, projectName: "fatma" },
