...
import type { DesktopUpdateActionResult, DesktopUpdateState } from "@fatma/contracts";
import { autoUpdater } from "electron-updater";

import type { ContextMenuItem } from "@fatma/contracts";
import { NetService } from "@fatma/shared/Net";
import { RotatingFileSink } from "@fatma/shared/logging";
...
const STATE_DIR =
  process.env.FATMA_STATE_DIR?.trim() || Path.join(OS.homedir(), ".fatma", "userdata");
const DESKTOP_SCHEME = "fatma";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = "fatma";
const APP_USER_MODEL_ID = "com.dinweldik.fatma";
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { fatmaCommitHash?: unknown };
    return normalizeCommitHash(parsed.fatmaCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.FATMA_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("fatma failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.FATMA_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.FATMA_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() ||
...
function backendEnv(): NodeJS.ProcessEnv {
  return {
    ...
    FATMA_MODE: "desktop",
    FATMA_NO_BROWSER: "1",
    FATMA_PORT: String(backendPort),
    FATMA_STATE_DIR: STATE_DIR,
    FATMA_AUTH_TOKEN: backendAuthToken,
  };
}
...
async function bootstrap(): Promise<void> {
  ...
  process.env.FATMA_DESKTOP_WS_URL = backendWsUrl;
