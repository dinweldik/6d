...
const port = Number(process.env.ELECTRON_RENDERER_PORT ?? 5774);
...
function cleanupStaleDevApps() {
  ...
  spawnSync("pkill", ["-f", "--", `--fatma-dev-root=${desktopDir}`], { stdio: "ignore" });
}
...
function startApp() {
  ...
  const app = spawn(
    resolveElectronPath(),
    [`--fatma-dev-root=${desktopDir}`, "dist-electron/main.js"],
    {
      cwd: desktopDir,
      env: {
        ...childEnv,
