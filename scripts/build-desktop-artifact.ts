...
interface StagePackageJson {
  ...
  readonly fatmaCommitHash: string;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "FATMA_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("FATMA_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "FATMA_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("FATMA_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("FATMA_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("FATMA_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("FATMA_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("FATMA_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("FATMA_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
});
...
function resolveGitHubPublishConfig():
  ...
  | undefined {
  const rawRepo =
    process.env.FATMA_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: "com.dinweldik.fatma",
    productName,
    artifactName: "fatma-${version}-${arch}.${ext}",
    directories: {
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `fatma-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    fatmaCommitHash: commitHash,
    private: true,
    description: "fatma desktop build",
    author: "fatma",
    main: "apps/desktop/dist-electron/main.js",
    build: yield* createBuildConfig(
      options.platform,
      options.target,
      desktopPackageJson.productName ?? "fatma",
      options.signed,
    ),
    dependencies: {
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: FATMA_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: FATMA_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: FATMA_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: FATMA_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: FATMA_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: FATMA_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: FATMA_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: FATMA_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: FATMA_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for fatma."),
