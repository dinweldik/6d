...
- Dev commands default `FATMA_STATE_DIR` to `~/.fatma/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --state-dir ~/.fatma/another-dev-state`
...
- Desktop production windows load the bundled UI from `fatma://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the fatma backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `FATMA_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `FATMA_DEV_INSTANCE`)
- Example: `FATMA_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `FATMA_PORT_OFFSET` to a numeric offset.
